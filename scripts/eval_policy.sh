...
# CUDA_VISIBLE_DEVICES renumbers GPUs, whereas Vulkan/Kit uses physical IDs.
...
if [[ "${isaac_physical_gpu_mode}" == "physical" ]]; then
  KIT_ARGS+=" --/renderer/activeGpu=${device_id} --/physics/cudaDevice=${device_id} --/renderer/multiGpu/enabled=false"
fi
