...
from gr00t.utils.video_utils import get_frames_by_indices, _get_video_info_ffmpeg
...
class LeRobotEpisodeLoader:
    ...
    def get_episode_length(self, idx: int) -> int:
        """Get the synchronized episode length used for training."""
        episode_id = self.episodes_metadata[idx]["episode_index"]
        return self._get_synchronized_episode_length(
            episode_id, self.episode_lengths[idx]
        )

    def _get_synchronized_episode_length(
        ...
        if "video" in self.modality_configs:
            ...
            for image_key in self.modality_configs["video"].modality_keys:
                nb_frames = self._get_video_frame_count(episode_id, image_key)
                if nb_frames is not None:
                    video_lengths.append(nb_frames)
            if video_lengths:
                actual_length = min(actual_length, min(video_lengths))
    ...
    def _resolve_video_path(self, episode_index: int, image_key: str) -> Path | None:
    ...
    def _get_video_frame_count(self, episode_index: int, image_key: str) -> int | None:
        video_path = self._resolve_video_path(episode_index, image_key)
        if video_path is None or not video_path.exists():
            return None

        info = _get_video_info_ffmpeg(str(video_path))
        nb_frames = int(info.get("nb_frames") or 0)
        return nb_frames if nb_frames > 0 else None
    ...
    def __getitem__(self, idx: int) -> pd.DataFrame:
        ...
        actual_length = self._get_synchronized_episode_length(episode_id, actual_length)
