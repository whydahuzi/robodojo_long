...
def encode_video_frames(
    ...
    # Get input frames. NOTE: DEFAULT_IMAGE_PATH writes 'frame-NNNNNN.jpeg' (hyphen),
    # so the glob/sort must match that, not the legacy 'frame_NNNNNN.jpeg'.
...
def concatenate_video_files(paths, output_path) -> None:
    """Losslessly concatenate mp4 files (same codec/params) into output_path.

    NOTE: referenced by lerobot_dataset_v3.save_episode but not defined upstream
    in this offline-vendored copy. Implemented here via the ffmpeg concat demuxer
    (stream copy). output_path may be one of the inputs, so we write to a temp file
    ...
    tmp_out = out + ".concat_tmp.mp4"
    try:
        with os.fdopen(list_fd, "w") as f:
            for p in abs_paths:
                f.write(f"file '{p}'\n")
        cmd = [ffmpeg, "-y", "-f", "concat", "-safe", "0", "-i", list_path, "-c", "copy", tmp_out]
        result = subprocess.run(cmd, capture_output=True, text=True)
        if result.returncode != 0:
            raise RuntimeError(f"ffmpeg concat failed:\n{result.stderr[-2000:]}")
        os.replace(tmp_out, out)
    finally:
        for p in (list_path, tmp_out):
            if os.path.exists(p):
                os.remove(p)
...
def get_video_duration_in_s(video_path: Path | str) -> float:
    ...
    NOTE: referenced by lerobot_dataset_v3.save_episode but not defined upstream
    in this offline-vendored copy, so it is implemented here via PyAV.
    """
    logging.getLogger("libav").setLevel(av.logging.ERROR)
