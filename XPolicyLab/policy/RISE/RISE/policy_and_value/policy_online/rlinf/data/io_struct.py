...
class EnvOutput:
    ...
    def prepare_observations(self, obs: dict[str, Any]) -> dict[str, Any]:
        ...
        if self.simulator_type == "roborl":
            return {
                "images": None,
                "wrist_images": None,
                "states": None,
                "task_descriptions": None,
                "lerobot_obs": obs["lerobot_obs"],
                "lerobot_gt_actions": obs["gt_actions"],
                "lerobot_history_obs": obs["history_obs"],
            }
        else:
...
class EmbodiedRolloutResult:
    ...
    wm_steps: Optional[torch.Tensor] = field(default_factory=list)
    ...
    def __post_init__(self):
        ...
        self.wm_steps = (
            [wm_step.cpu().contiguous() for wm_step in self.wm_steps]
            if self.wm_steps is not None
            else []
        )
        self.rewards = (
    ...
    def append_result(self, result: dict[str, Any]):
        ...
        self.wm_steps.append(
            result["wm_steps"].cpu().contiguous()
        ) if "wm_steps" in result else []
        self.rewards.append(
    ...
    def to_dict(self):
        ...
        # RuntimeError: stack expects each tensor to be equal size, but got [8, 10] at entry 0 and [8, 50] at entry 1
        rollout_result_dict["wm_steps"] = (
            torch.stack(self.wm_steps, dim=0).cpu().contiguous()
            if len(self.wm_steps) > 0
            else None
        )
        ...
        for k in merged_forward_inputs.keys():
            assert k not in ["dones", "rewards", "prev_logprobs", "prev_values", "wm_steps"]
