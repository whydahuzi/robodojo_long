...
def _collate(tensors, max_sequence_length=None, dtype=None, pad=False, pad_value=-1):
    # , max_len as batch column, fromonly pad,
...
class MMCollatorForAction:
    ...
    def __call__(self, batch: List[Dict[str, Any]]) -> Dict[str, Any]:
        ...
        # ifadd action tokens, firstprocess input_tokens
        ...
        action_list = [torch.from_numpy(ex["action"].copy()) for ex in batch] # add action
        # action columnConvert to tensor
    ...
    def _add_proprio_tokens_to_batch(self, batch):
        ...
        for ex in batch:
            ...
            ex['proprio_token_idx'] = max_input_length  # proprio token index
    ...
    def _add_action_tokens_to_batch(self, batch):
        ...
        for ex in batch:
            ...
            if "loss_masks" in ex:
                ...
                # as action tokens add loss mask (as 0, Compute loss)
            ...
            if "subsegment_ids" in ex:
                ...
                # as action tokens subsegment id
                ...
                # synchronize position_ids(fix: as action Position indices)
                ...
                # if subsegment_ids, byforgenerate
...
class DiTActionCollator:
    ...
    def __call__(self, batch: List[Dict[str, Any]]) -> Dict[str, Any]:
        ...
        # processactiondata
        action_list = [torch.from_numpy(ex["action"]) for ex in batch] # add action
        # action columnConvert to numpy
        out['action']= torch.stack(action_list, dim=0)
