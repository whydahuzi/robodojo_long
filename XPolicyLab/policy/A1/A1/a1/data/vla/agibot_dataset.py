...
def _concat_action_step(
    ...
    if eff_pos_t.ndim == 1 and eff_pos_t.shape[0] == 2:
        ...
        # If multi-channel, use channel 0 as the gripper as a fallback
...
class AgiBotWorldAlphaDataset(Dataset):
    ...
    # Specify available frame ranges through the filtered_frame_ranges.xlsx generated by lfwj_range.py
    frame_ranges_excel: Optional[str] = None  # Specified path; if None, try to auto-discover it in root_dir or CWD

    # Performance-related config; cache implementation was removed, keeping the minimal read path
    ...
    def __post_init__(self):
        ...
        # Some repositories store actions in Actiondata/proprio_stats
        ...
        # Load the frame-range Excel exported by agibot_range.py
        self._frame_ranges_map: Optional[Dict[Tuple[str, int], Tuple[int, int]]] = self._load_frame_ranges_excel()

        # Prebuild the available sample index: [(set_id, episode_id, frame_idx)]
    ...
    def _read_action_chunk(self, h5: h5py.File, start_idx: int, length: int) -> Optional[np.ndarray]:
        ...
        # Process gripper, using channel 0
        ...
        # Align to the fixed ACTION dimension
    ...
    def _read_proprio(self, h5: h5py.File, idx: int) -> Optional[np.ndarray]:
        # 16-dimensional format matching the action: end pos(3)+quat(4)+gripper(1) for right; left; right first, left second
        try:
    ...
    def get(self, item: int, rng) -> Dict[str, Any]:
        ...
        if self.use_wrist_image:
            ...
            if primary_img is None:
                # This should have been filtered during indexing, but keep a fallback here
                raise IndexError("Primary image missing for sample.")

        # Action + proprioception, without caching; open and close uniformly
        ...
        try:
            ...
            # Do not close proactively; cache management handles it, and h5py handles are reused and released on process exit
            pass
    ...
    def _load_frame_ranges_excel(self) -> Optional[Dict[Tuple[str, int], Tuple[int, int]]]:
        ...
        for _, row in df.iterrows():
            try:
                ...
                # If duplicates exist, merge to the minimum start and maximum end
                if key in frame_map:
