...
from lerobot.policies.pi0_fast.transform_pi0_fast import (
    PI0FastGemmaTokenizerTransformFn, 
    UnifyPI0FastInputsTransformFn,
    FASTActionTokenizerTransformFn,
)
...
@DatasetConfig.register_subclass("pi0_fast")
@dataclass
class PI0FastDatasetConfig(DatasetConfig):
    """Dataset configuration for PI0Fast policy.
    
    PI0Fast uses discrete action token prediction (cross-entropy loss)
    instead of flow matching. The data transforms prepare:
    - State discretization into 256 bins
    - FAST action token encoding
    """
    height: int = 224
    width: int = 224
        ...
        default_factory=lambda: TransformGroup(
            inputs=[
                ...
                ResizeImagesWithPadFn(
                    height=PI0FastDatasetConfig.height, 
                    width=PI0FastDatasetConfig.width, 
                ),  # ✅  
                NormalizeTransformFn(),  # ✅
                ComposeFieldsTransform(),  # ✅
                PI0FastGemmaTokenizerTransformFn(),  # ✅
                PadStateAndActionTransformFn(
                    max_state_dim=PI0FastDatasetConfig.max_state_dim, 
                    max_action_dim=PI0FastDatasetConfig.max_action_dim, 
                ),  # ✅
...
class PI0FastConfig(PreTrainedConfig):
    """Configuration for PI0Fast policy.
    
    PI0Fast is a variant of PI0 that uses discrete action token prediction
    (FAST tokenizer) instead of flow matching. This enables:
    - Faster inference via autoregressive decoding
    ...
    normalization_mapping: dict[str, NormalizationMode] = field(
        default_factory=lambda: {
            ...
            "STATE": NormalizationMode.MEAN_STD,  # PI0Fast uses MEAN_STD for state
            "ACTION": NormalizationMode.MEAN_STD,  # PI0Fast uses MEAN_STD for action
        }
    )
