...
from ahawam.utils.pytorch_utils import dict_apply
from ahawam.utils.logging_config import get_logger
from .base_processor import BaseProcessor

logger = get_logger(__name__)

class AHAWAMProcessor(BaseProcessor):
    def __init__(
        self,
        # keys
    ...
    def preprocess(self, data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        for meta in self.shape_meta["images"]:
            ...
            assert list(image.shape[1:]) == shape, \
                f"Expected image spatial/channel shape {shape}, got {image.shape[1:]} after transforms for key {key}"
